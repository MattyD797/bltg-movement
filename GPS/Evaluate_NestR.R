...
BTGO_GPS_Haanmeer_output_1 <- find_nests(gps_data = BTGO_GPS_Haanmeer_nestR,
                            sea_start = "03-01",
                            sea_end = "07-31",
                            nest_cycle = 27,
                            buffer = 30,
...
individual <- read.csv("./UvA_Transmitter_info.csv"); head(individual)
nest <- read.csv("./UvA_nest_locs.csv");head(nest)

#now, join the nest and individual datasets first
nesters <- right_join(nest, individual, by = "NestID"); head(nesters)

#change ID to factor
nesters$burst <- as.factor(nesters$TransmitterID)
