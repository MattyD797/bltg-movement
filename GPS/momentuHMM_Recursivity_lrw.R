...
foo( c("dplyr", "sp", "ggplot2", "ggmap", "plyr", "lubridate", "SDLfilter", "moveHMM", "momentuHMM", "bsam", "parallel", "adehabitatLT", "recurse"))
...
test <- filter(data, ID == "1009-2013" | ID ==  "2015-2013")
...
recursions_all = getRecursions(test, radius = 7)

test <- cbind(test, recursions_all$revisits)
test <- cbind(test, recursions_all$residenceTime)
head(test)
