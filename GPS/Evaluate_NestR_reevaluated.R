...
# Script for processing the GPS data from Verhoeven et al. 2020 and calculating summary statistics on kernel area
...
options(scipen = 999)
memory.limit(350000) #allow R to allocate vectors > 1MB
#options(max.print=999999) #if you need to see the complete output
#


#read in the data
movedata <- read.csv("./GPS_BTGO_Haanmeer_nestR.csv"); head(movedata)
movedata <- na.omit(movedata)

#prep for nestR
burst <- movedata$burst
long <- movedata$long
lat <-  movedata$lat
date <- as.POSIXct(movedata$date, tx = "GMT", format = "%m/%d/%Y %H:%M:%S")

BTGO_GPS_Haanmeer_nestR <- data.frame("burst" = burst,"long" =long,"lat" = lat,"date" =date)

BTGO_GPS_Haanmeer_nestR <- na.omit(BTGO_GPS_Haanmeer_nestR)
BTGO_GPS_Haanmeer_nestR_filter <- BTGO_GPS_Haanmeer_nestR %>% filter(burst %in% c("2002-2013","2004-2013","2014-2013","2017-2013","2011-2013","2018-2013","2042-2013","2006-2013","1001-2012","2012-2013","2013-2013","2019-2013","2001-2013","2002-2014","2002-2015","2015-2014","2016-2013","2018-2014"))

unique(BTGO_GPS_Haanmeer_nestR_filter$burst)
...
BTGO_GPS_Haanmeer_output_1 <- find_nests(gps_data = BTGO_GPS_Haanmeer_nestR_filter,
                            sea_start = "03-01",
                            sea_end = "07-31",
                            nest_cycle = 27,
                            buffer = 13,
...
head(BTGO_GPS_Haanmeer_output_1$nests)
...
individual <- read.csv("C:/Users/14064/Dropbox/BTGO Movmement Study/BTGO git/bltg-movement/CSV/GPS Data/UvA_Transmitter_info.csv"); head(individual)
nest <- read.csv("C:/Users/14064/Dropbox/BTGO Movmement Study/BTGO git/bltg-movement/CSV/GPS Data/UvA_nest_locs.csv");head(nest)

#now, join the nest and individual datasets first
nesters <- right_join(nest, individual, by = "NestID"); head(nesters)

# #change ID to factor
 nesters$ID <- as.factor(nesters$TransmitterID)
# 
# 
...
#filter only burst with known nest location - for training purposes
BTGO_known_nests<-nesters[!is.na(nesters$Easting),]

#define coords and projection - currently in UTM
coordinates(BTGO_known_nests) <- c("Easting", "Northing")
...
colnames(BTGO_known_nests_locs) <- c("long", "lat"); head(BTGO_known_nests_locs)

known_nests_burst <- paste(BTGO_known_nests$ID, BTGO_known_nests$YearDeployed, sep = "-")

BTGO_known_nests1 <- data.frame("burst" = as.character(known_nests_burst), "fate" = BTGO_known_nests$NestFate, "long" = as.numeric(BTGO_known_nests_locs$long), "lat" = as.numeric(BTGO_known_nests_locs$lat), "start_known" = BTGO_known_nests$FirstTimeFound, "end_known" = BTGO_known_nests$HatchDate); head(BTGO_known_nests1)
...
BTGO_GPS_Haanmeer_output_1$nests
BTGO_GPS_Haanmeer_output_1$visits
table(BTGO_GPS_Haanmeer_output_1$nests$burst)


# write a for loop for this - after practice!!!!

output1 <- BTGO_GPS_Haanmeer_output_1$nests %>%
...
# 
(explodata <- get_explodata(candidate_nests = BTGO_GPS_Haanmeer_output_1$nests, 
...
#we want a training faction of 8:1:1 (.8 for training)

(BTGO_cart <- discriminate_nests(explodata = explodata, train_frac = .8))
...
FPR <- 0.06; TPR <- (1 - FPR); FNR <- .17; TNR <- (1 - FNR)

(A <- (TPR + TNR)/(TPR + TNR + FPR + FNR))
...
BTGO_Haanmeer_output_2 <- find_nests(gps_data = BTGO_GPS_Haanmeer_nestR_filter,
                                     sea_start = "03-01",
                                     sea_end = "07-31",
                                     nest_cycle = 27,
                                     buffer = 13,
...
#I choose a p_time bc we know that godwits are easier and easier to find as they are on the nest longer, but that their survival may not change through time.
...
plot_nest_surv(BTGO_outcomes, who = 18)

# output of Bayes survival model
out <- summarize_outcomes(BTGO_outcomes, ci = .95)$outcomes

head(individual); individual$birdyear <- paste(individual$TransmitterID, individual$YearDeployed, sep="-")

out$birdyear <- substr(out$burst,1,9)

check <- right_join(out, individual, by = "birdyear")

head(check)

 # write.csv(check, "C:/Users/14064/Dropbox/BTGO Movmement Study/BTGO git/bltg-movement/CSV/output/nestR_attempt_03.28.21_output.csv")
