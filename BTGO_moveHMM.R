...
ggplot(hmmdata, aes(step)) + geom_density(color = "red", fill = "red", alpha = 0.3) + theme_classic() + coord_cartesian(xlim=c(0,100)) 
...
mu0 <- c(0.01,0.7) # step mean (two parameters: one for each state) in km. state 1 involving relatively short steps and many turnings, state 2 involving loger steps and fewer turnings
sigma0 <- c(0.01,0.7) # step SD
